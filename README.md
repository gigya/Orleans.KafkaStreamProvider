...
It works with a modified Kafka-Net library which is a native C# client for Apache Kafka (for more information see https://github.com/EranOfer/kafka-net)
...
- **NumOfQueues**: The number of queues the Provider will use. *Default value is 4*
- **CacheSize**: The size of each queue's cache. The value represents the number of messages that can be stored in the cache. *Default value is 16384*.
...
- **MaxMessagesToTakeFromKafka** - The maximal number of messages for the Receiver to ask for Kafka in one fetch. Used as an upper bound in the receiver. *Default value is 65536*.
